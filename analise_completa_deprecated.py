...
def analisar_sentimento_noticias(ticker_symbol="ITUB"):
    ...
    try:
        ...
        with open(f'{ticker_symbol}_sentimento.json', 'w', encoding='utf-8') as f:
...
def atualizar_dados_yfinance(ticker="ITUB4.SA", start="1995-01-01", end=None, arquivo="historico.csv"):
    if end is None:
        end = calcular_data_fim_padrao()
...
def main():
    ...
    parser.add_argument('--ticker', type=str, default="ITUB4", help="Ticker da B3 (ex: PETR4)")
    ...
    if args.modo_streamlit:
        ...
        ticker_yf = f"{args.ticker}.SA" if not args.ticker.endswith(".SA") else args.ticker
        ticker_clean = args.ticker.replace(".SA", "")
        arquivo_historico = f"{ticker_clean}_historico.csv"
        
        atualizar_dados_yfinance(ticker=ticker_yf, start=args.start, end=args.end, arquivo=arquivo_historico)
        processador = ProcessadorDados(arquivo_historico)
        ...
        # Salvar resultados
        df_historico.to_csv(f'{ticker_clean}_processado_final.csv', index=False)
        if len(df_previsoes) > 0:
            df_previsoes.to_csv(f'{ticker_clean}_previsoes_finais.csv', index=False)
        ...
        with open(f'{ticker_clean}_metricas.json', 'w') as f:
            json.dump(metricas_serializable, f, indent=2)
            
        # 8. Analisar Sentimento de Noticias
        analisar_sentimento_noticias(ticker_clean)
